...
class BackgroundRemoverApp:
    def __init__(self, root):
        ...
        self.root.geometry("600x580")  # Increased height to accommodate new options
        ...
        self.total_images = 0
        self.models_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Models")
        self.model_path = os.path.join(self.models_folder, "RMBG-2.0")
    ...
    def initialize_model(self):
        ...
        try:
            ...
            if not os.path.exists(self.model_path) or not os.listdir(self.model_path):
                loading_label.config(text="Downloading model (this may take a while)...")
                self.root.update()
                self.download_model()
                
            loading_label.config(text="Loading model from local folder...")
            self.root.update()
            
            self.model = AutoModelForImageSegmentation.from_pretrained(self.model_path, local_files_only=True, trust_remote_code=True)
    ...
    def download_model(self):
        """Download the model from Hugging Face to local folder"""
        try:
            snapshot_download(repo_id="briaai/RMBG-2.0", local_dir=self.model_path)
    ...
    def create_widgets(self):
        ...
        model_frame = tk.LabelFrame(content_frame, text="Model Settings", 
                            bg="#f0f4f8", fg="#2a6099", padx=10, pady=10,
                            font=("Arial", 10, "bold"))
        model_frame.pack(fill="x", pady=10)
        
        # Model path display
        model_path_frame = tk.Frame(model_frame, bg="#f0f4f8")
        model_path_frame.pack(fill="x", pady=5)
        
        model_path_label = tk.Label(model_path_frame, text="Model Path:", width=15, anchor="w", 
                            bg="#f0f4f8", fg="#333333")
        model_path_label.pack(side="left")
        
        self.model_path_var = StringVar()
        self.model_path_var.set(self.model_path)
        model_path_display = tk.Label(model_path_frame, textvariable=self.model_path_var, 
                              bg="white", fg="#555555", padx=5, pady=2, width=40, anchor="w")
        model_path_display.pack(side="left", padx=5)
        
        # Model update button
        update_model_button = tk.Button(model_path_frame, text="Update Model", command=self.update_model,
                               bg="#4a86cf", fg="white", padx=10)
        update_model_button.pack(side="left")
        ...
        input_label = tk.Label(input_frame, text="Input Folder:", width=15, anchor="w", 
                            bg="#f0f4f8", fg="#333333")
    ...
    def update_model(self):
        ...
        if messagebox.askyesno("Update Model", "Do you want to download the latest model from Hugging Face?"):
            try:
                # Store old status
                old_status = self.status_var.get()
                self.status_var.set("Downloading latest model...")
                self.root.update()
                
                # Remove existing model folder content
