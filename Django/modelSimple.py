# --- START OF FILE simplified_models.py ---

import uuid
from django.db import models
# from django.contrib.auth.models import User # Removed as UserProfile/Contragent are gone
...
class BaseModel(models.Model):
    ...
    # is_deleted = models.BooleanField(default=False, db_index=True) # Optional: Keep if soft delete is desired
...
class Image(BaseModel):
    ...
    def save(self, *args, **kwargs):
        # Basic image processing can be kept or removed for ultimate simplicity
        ...
        if (is_new or 'file_path' in kwargs.get('update_fields', [])) and self.file_path:
            try:
                ...
                # Clear metadata if file not found?
                # Image.objects.filter(pk=self.pk).update(resolution=None, size=None, format=None)
...
class Category(BaseModel):
    ...
    # name2 = models.CharField(max_length=255, blank=True, null=True, help_text="Alternative name (e.g., Russian)") # Optional
    ...
    # meta_title = models.CharField(max_length=255, blank=True, null=True) # Optional SEO fields
    # meta_description = models.TextField(blank=True, null=True) # Optional SEO fields
...
class Price(BaseModel):
    ...
    is_active = models.BooleanField(default=True, help_text="Is this price currently active?") # Useful flag

    class Meta(BaseModel.Meta):
        ordering = ['-is_active', 'amount'] # Show active prices first, then by amount
        indexes = [
...
class Product(BaseModel):
    ...
    # Direct link to a default price or rely on querying related Price objects
    # Option 1: Select a default price instance
    # default_price = models.ForeignKey(Price, null=True, blank=True, on_delete=models.SET_NULL, related_name='+') # '+' avoids reverse relation clutter
    ...
    def current_price_display(self):
        ...
        # Finds the first price marked as active for this product
        active_price = self.prices.filter(is_active=True).order_by('amount').first()
        if active_price:
            return f"{active_price.amount} {active_price.currency}"
        # Fallback: maybe find any price if no active one exists? (optional)
        # any_price = self.prices.order_by('amount').first()
        # if any_price:
        #     return f"{any_price.amount} {any_price.currency} (Inactive)"
...
# --- END OF FILE simplified_models.py ---