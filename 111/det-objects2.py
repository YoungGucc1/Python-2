...
def draw_boxes_on_image(image: cv2.Mat, detections: List[Dict[str, Any]]) -> cv2.Mat:
    ...
    for detection in detections:
        try:
            ...
            label_bg_y1 = box[1] - text_height - baseline - 2 # Adjust spacing
            label_bg_y2 = box[1] - baseline + 2
            # Ensure background is within image bounds (top)
            if label_bg_y1 < 0:
                label_bg_y1 = box[1] + baseline + 2 # Draw below box start if too close to top
                label_bg_y2 = box[1] + text_height + baseline + 2

            cv2.rectangle(img_copy, (box[0], label_bg_y1), (box[0] + text_width, label_bg_y2), color, -1) # Filled background

            # Put label text
            label_y = label_bg_y1 + text_height # Y position for the text baseline
            cv2.putText(img_copy, label, (box[0] + 2), label_y,
