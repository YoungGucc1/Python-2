    ...
    def run(self):
        ...
        for img_path in self.image_paths:
            try:
                ...
                if image is None:
                    self.status_update.emit(f"Error loading {os.path.basename(img_path)}: File not readable")
                    continue
                else:
                    self.status_update.emit(f"Loaded {os.path.basename(img_path)} successfully")
                ...
                try:
                    # Use DeepFace.detect_faces instead of extract_faces for more reliable detection
                    detected_faces = DeepFace.detect_faces(
                        img_path=img_path,
                        detector_backend='retinaface'  # Try a more reliable backend first
                    )

                    # Check if faces were detected
                    if detected_faces and len(detected_faces) > 0:
                        self.status_update.emit(f"Found {len(detected_faces)} faces in {os.path.basename(img_path)}")
                        ...
                        for face_obj in detected_faces:
                            try:
                                # Get the facial area (different format from what you had)
                                face_area = face_obj.get('facial_area', face_obj.get('box', {}))
                                if face_area:
                                    # Extract location (depends on detector backend format)
                                    if isinstance(face_area, dict):
                                        x = face_area.get('x', 0)
                                        y = face_area.get('y', 0)
                                        w = face_area.get('w', 0)
                                        h = face_area.get('h', 0)
                                    ...
                                    # Extract just the face region from the original image
                                    face_img = image[y:y+h, x:x+w]
                                    
                                    # Save the face to a temporary file for DeepFace
                                    temp_face_path = f"temp_face_{face_obj.get('face_id', i)}.jpg"
                                    cv2.imwrite(temp_face_path, face_img)
                                    
                                    # Get the embedding using the standard approach
