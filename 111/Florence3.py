...
# --- Try to import qwen-vl-utils for better Qwen model support ---
try:
    ...
    try:
        ...
        print("Qwen VL Utils installed successfully via pip.")
        import qwen_vl_utils
    except Exception as e:
        print(f"Could not install or import qwen-vl-utils: {e}\nApp will continue without it.")
# --- End Optional Installation ---
...
FALLBACK_MODEL = "Qwen/Qwen-VL-Chat"  # Fallback model if default fails
AVAILABLE_MODELS = [
    ...
    "Qwen/Qwen-VL-Chat",  # Added as an alternative that may be more stable
]
    ...
    if model_name not in models:
        try:
            ...
            if "Qwen" in model_name and "VL" in model_name:
                ...
                # Try to manually add the missing function if needed
                try:
                    import transformers.utils
                    if not hasattr(transformers.utils, 'is_quark_available'):
                        print("Adding missing 'is_quark_available' function to transformers.utils")
                        # Add the missing function (returns False as a placeholder)
                        transformers.utils.is_quark_available = lambda: False
                except Exception as patch_e:
                    print(f"Warning: Failed to patch transformers.utils: {patch_e}")
                
                # Try alternative loading approaches
                try:
                    # First, try to directly load via pipeline which sometimes works better with missing dependencies
                    from transformers import pipeline
                    print("Attempting to load via pipeline...")
                    image_to_text = pipeline("image-to-text", model=model_name, device=0 if device.type == 'cuda' else -1)
                    # Extract model and processor from pipeline
                    model = image_to_text.model
                    processor = image_to_text.processor
                    print("Successfully loaded via pipeline")
                except Exception as pipe_ex:
                    print(f"Pipeline approach failed: {pipe_ex}")
                    
                    try:
                        # Second attempt: try direct class import
                    ...
                    except (ImportError, Exception) as direct_ex:
                        print(f"Direct class import failed: {direct_ex}")
                        
                        # Third attempt: try using AutoModel
                        try:
                            # Try using AutoModel with a device map for efficient loading
                            print("Trying AutoModel approach...")
                            ...
                            print(f"AutoModel approach failed: {auto_ex}")
                            
                            # Last resort: try using an alternative Qwen model
                            print("Attempting to use an alternative Qwen model...")
                            alt_model_name = "Qwen/Qwen-VL-Chat" # Try older but compatible model
                            model = AutoModelForCausalLM.from_pretrained(
                                alt_model_name,
                                trust_remote_code=True,
                                torch_dtype=dtype
                            ).eval().to(device)
                            model_name = alt_model_name # Update model name for processor loading
                            print(f"Using alternative model: {alt_model_name}")
                
                # Load the processor separately since it might have fewer dependencies
                print(f"Loading processor: {model_name}...")
                try:
                    processor = AutoProcessor.from_pretrained(model_name, trust_remote_code=True)
                except Exception as proc_ex:
                    print(f"Error loading processor directly: {proc_ex}")
                    # Try a workaround for processor loading
                    if "Qwen" in model_name:
                        try:
                            from transformers import AutoTokenizer
                            print("Loading tokenizer as alternative...")
                            tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
                            # Create a simple wrapper that mimics the processor interface
                            class SimpleProcessor:
                                ...
                                def __call__(self, text=None, images=None, return_tensors=None):
                                    # Basic processing
                                    inputs = self.tokenizer(text, return_tensors=return_tensors)
                                    if images is not None:
                                        # This is a placeholder - real implementation would depend on model
                                        inputs["pixel_values"] = torch.zeros((1, 3, 224, 224), device=device)
                                    return inputs
                            ...
                            print("Created simple processor wrapper")
                        except Exception as tok_ex:
                            print(f"Tokenizer fallback also failed: {tok_ex}")
                            raise
            else:
...
class Worker(QObject):
    ...
    def run(self):
        ...
        is_qwen_model = "Qwen" in self.model_name
        ...
        for i, img_path in enumerate(self.image_paths):
            ...
            try:
                ...
                if is_qwen_model:
                    # For Qwen models, use the chat template format
                    try:
                        messages = [
                        ...
                        # Apply chat template if the method exists
                        ...
                        # Use half precision on GPU if needed
                        if device.type == 'cuda' and model.dtype == torch.float16:
                            if 'pixel_values' in inputs:
                                inputs['pixel_values'] = inputs['pixel_values'].half()
                        
                        # Try different generate methods based on model type
                        try:
                            # First try the standard generate method
                            ...
                            # Get only new tokens (remove input tokens)
                            if len(generated_ids.shape) > 1:  # If batch dimension exists
                            ...
                            if hasattr(processor, 'decode'):
                                generated_text = processor.decode(generated_ids_trimmed, skip_special_tokens=True)
                            else:
                                generated_text = processor.tokenizer.decode(generated_ids_trimmed, skip_special_tokens=True)
                                
                            caption = generated_text.strip()
                        except Exception as model_ex:
                            # If standard generate fails, try alternative approach
                            self.log.emit(f"Warning: Standard generation failed, trying alternative: {model_ex}")
                            
                            # Try using a different method or attribute
                            if hasattr(model, 'chat'):
                                response = model.chat(processor, messages)
                                caption = response.strip()
                            else:
                                # Last resort fallback
                                raise Exception(f"Could not generate with model: {model_ex}")
                    
                    except Exception as qwen_ex:
                        self.log.emit(f"Error with Qwen-specific processing: {qwen_ex}. Trying generic approach.")
                        # Fall back to generic processing if Qwen-specific fails
                        ...
                        generated_text = processor.batch_decode(generated_ids_cpu, skip_special_tokens=True)[0]
                        caption = generated_text
                        
                else:
                    # Original Florence/other models handling
                    task_prompt = "<DESCRIPTION>"
                    prompt = task_prompt + "Describe this image in great detail."
                    ...
                    try:
                        ...
                        self.log.emit(f"Warning: Error in post-processing: {parse_ex}")
                        caption = generated_text
...
class CaptioningApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Florence/Qwen Batch Captioner (PyQt6)")
        ...
        if not load_model_processor(DEFAULT_MODEL):
            ...
            print(f"Default model failed to load, trying fallback model: {FALLBACK_MODEL}")
            QMessageBox.warning(
                self, 
                "Model Load Warning", 
                f"Failed to load default model {DEFAULT_MODEL}.\n\nTrying fallback model {FALLBACK_MODEL}..."
            )
            
            if not load_model_processor(FALLBACK_MODEL):
                # If even the fallback fails, try any other model in the list
                QMessageBox.warning(
                    self, 
                    "Model Load Warning", 
                    f"Fallback model also failed. Will try other models in the list."
                )
                
                # Try each model until one works
                loaded_any = False
                for model_name in AVAILABLE_MODELS:
                    if model_name != DEFAULT_MODEL and model_name != FALLBACK_MODEL:
                        print(f"Trying alternative model: {model_name}")
                        if load_model_processor(model_name):
                            QMessageBox.information(
                                self,
                                "Model Loaded",
                                f"Successfully loaded alternative model: {model_name}"
                            )
                            loaded_any = True
                            # Update selected model in UI (will be set after initUI)
                ...
                if not loaded_any:
                    QMessageBox.critical(
                        self, 
                        "Critical Error", 
                        "Failed to load any models. Application may not function properly."
                    )
                    # We'll continue but warn the user that things might not work
                    self.current_model = DEFAULT_MODEL  # Keep this as the UI default even though it failed
            else:
                # Fallback model worked
                self.current_model = FALLBACK_MODEL
        else:
            # Default model worked
            self.current_model = DEFAULT_MODEL

        self.initUI()
        self.apply_stylesheet() # Apply dark theme
        
        # Set the correct model in the UI after initialization
        if hasattr(self, 'current_model') and hasattr(self, 'model_combo'):
