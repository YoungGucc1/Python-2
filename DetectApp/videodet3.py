...
def find_available_models():
    ...
    for model_file in model_files:
        ...
        models_dict[display_name] = str(Path("Models") / model_file.name)
        print(f"Found model: {display_name} -> {models_dict[display_name]}")
...
# Fallback model definitions if no models found
if not MODEL_FILES:
    MODEL_FILES = {
        "YOLOv8n": "yolov8n.pt",
        "YOLOv8s": "yolov8s.pt",
...
# Set default model (first model in the dictionary, or YOLOv8n if available)
DEFAULT_MODEL = next(iter(MODEL_FILES.keys())) if MODEL_FILES else "YOLOv8n"
...
class ModelLoader(QObject):
    ...
    def run(self):
        try:
            if self.model_key not in MODEL_FILES:
                raise FileNotFoundError(f"Model key '{self.model_key}' not found in MODEL_FILES.")

            model_pt_path_str = MODEL_FILES[self.model_key]
            model_pt_path = Path(model_pt_path_str)

            if not model_pt_path.exists():
                raise FileNotFoundError(f"Model file not found: {model_pt_path_str}")
            ...
            if self.device != "cpu":
                if engine_path.exists():
                    ...
                    self.modelReady.emit(None, info_text)
            ...
            if not use_engine:
                ...
                if self.device != "cpu" and not engine_path.exists(): # Export if not CPU and engine wasn't found/loaded
                    try:
                        ...
                        exported_engine_path = Path(f"{model_pt_path.stem}.engine") # Default export location
                        if exported_engine_path.exists():
                            exported_engine_path.rename(engine_path) # Move to our engines dir
                            print(f"TensorRT engine exported and saved to: {engine_path}")
                            # Reload using the engine for subsequent runs
                            self.model = YOLO(engine_path, task='detect')
                            info_text = f"TensorRT engine created: {engine_path.name}"
                        else:
                             info_text += " (Engine export succeeded, but file not found at expected location)"
...
class ObjectDetectionApp(QMainWindow):
    def __init__(self):
        ...
        model_count = len(MODEL_FILES) if MODEL_FILES else 0
        if model_count > 0:
            self.status_bar.showMessage(f"Found {model_count} models in Models directory. Using {DEFAULT_MODEL} as default.")
        else:
            self.status_bar.showMessage("No models found in Models directory. Please add .pt files to the Models folder.")

        # --- Load Initial Model ---
        self.load_model() # Load default model on startup
    ...
    def setup_ui(self):
        ...
        # Populate dropdown with available models from MODEL_FILES
        if MODEL_FILES:
            self.model_combo.addItems(MODEL_FILES.keys())
        ...
        self.refresh_models_button = QPushButton(icon=self.style().standardIcon(QStyle.StandardPixmap.SP_BrowserReload), text="")
        self.refresh_models_button.setToolTip("Refresh models list")
        self.refresh_models_button.clicked.connect(self.refresh_models)
        self.refresh_models_button.setFixedSize(24, 24)  # Make it compact
    ...
    # --- Model Handling ---
    def on_model_change(self, model_key):
        if self.is_playing or self.is_paused:
             reply = QMessageBox.question(self, "Change Model?",
                                          "Changing the model requires stopping the current video. Proceed?",
                                          QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                          QMessageBox.StandardButton.No)
             if reply == QMessageBox.StandardButton.Yes:
                 self.stop_detection() # Stop playback first
                 self.load_model()
             else:
                 # Find the previous model key and reset combo (requires storing previous key)
                 # Or simply prevent change if not stopping - easier for now:
                 # Find the key associated with the currently loaded model if possible
                 current_model_key = DEFAULT_MODEL # Fallback
                 for key, path in MODEL_FILES.items():
                     if self.model and hasattr(self.model, 'ckpt_path') and self.model.ckpt_path and Path(path).name in str(self.model.ckpt_path):
                         current_model_key = key
                         break
                     elif self.model and hasattr(self.model, 'engine_path') and self.model.engine_path and Path(path).stem in str(self.model.engine_path):
                         current_model_key = key
                         break

                 self.model_combo.blockSignals(True)
                 self.model_combo.setCurrentText(current_model_key)
    ...
    def load_model(self):
        ...
        self.status_bar.showMessage(f"Loading model: {selected_model_key}...")
        self.info_label.setText(f"Loading {selected_model_key}...")
        self.play_pause_button.setEnabled(False)
        self.stop_button.setEnabled(False)
        self.save_button.setEnabled(False)
        self.model_combo.setEnabled(False)
        ...
        self.model_loader_worker = ModelLoader(selected_model_key, self.device)
    ...
    def on_model_ready(self, loaded_model, info_text):
        ...
        self.model_info = info_text
        self.info_label.setText(self.model_info)
        if self.model:
             self.status_bar.showMessage("Model loaded successfully.", 5000)
             print(f"Model ready: {info_text}")
             # Enable playback only if a video is also selected
             if self.video_path:
                 self.play_pause_button.setEnabled(True)
                 self.save_button.setEnabled(True) # Enable saving once model and video are ready
                 self.slider.setEnabled(True)
        else:
            # This might happen if TensorRT export started but didn't finish/fail yet
            self.status_bar.showMessage(info_text) # Show intermediate message
    ...
    def reset_controls_after_load(self):
        ...
        print("Model loader thread finished.")
        self.model_loader_thread = None # Allow garbage collection
        self.model_loader_worker = None
    ...
    def start_detection(self, start_frame=0):
        ...
        self.model_combo.setEnabled(False) # Prevent changing model while playing
    ...
    def refresh_models(self):
        """Rescan the Models folder for model files and update the dropdown."""
        global MODEL_FILES, DEFAULT_MODEL
        
        # Check if we can refresh (not allowed during playback or model loading)
        if self.is_playing or self.is_paused:
            QMessageBox.warning(self, "Cannot Refresh Models", 
                               "Please stop playback before refreshing models.")
            return
            
        if self.model_loader_thread and self.model_loader_thread.isRunning():
            QMessageBox.warning(self, "Model Loading in Progress", 
                               "Please wait for the current model to finish loading.")
            return
            
        # Get currently selected model name if any
        current_model = self.model_combo.currentText()
        
        # Update the global MODEL_FILES dictionary
        MODEL_FILES = find_available_models()
        
        # Update the default model
        if MODEL_FILES:
            DEFAULT_MODEL = next(iter(MODEL_FILES.keys()))
        else:
            DEFAULT_MODEL = "YOLOv8n"  # fallback
            
        # Update the dropdown
        self.model_combo.blockSignals(True)  # Prevent triggering model change event
        ...
        if MODEL_FILES:
            ...
            if current_model in MODEL_FILES:
                ...
                self.model_combo.setCurrentText(DEFAULT_MODEL)
            self.model_combo.setEnabled(True)
            self.status_bar.showMessage(f"Found {len(MODEL_FILES)} models in Models directory", 5000)
        else:
            self.model_combo.addItem("No models found")
            self.model_combo.setEnabled(False)
            self.status_bar.showMessage("No model files found in Models directory", 5000)
            
        self.model_combo.blockSignals(False)
        
        # If the model has changed or no previous model was loaded, load the new default
        if current_model != self.model_combo.currentText() or self.model is None:
            self.load_model()
